...
class PuriDivER:
    ...
    def before_task(self, datalist, init_model=False, init_opt=True):
        ...
        incoming_classes = incoming_df["label"].unique().tolist()
        ...
        print("exposed", len(self.exposed_classes), "learn", self.num_learning_class)
        '''
        for label in incoming_classes:
            if label not in self.label_map:
                if "true_label" in incoming_df:
                    label = incoming_df[incoming_df['label'] == label].iloc[0]['true_label']
                else:
                    label = incoming_df[incoming_df['label'] == label].iloc[0]['label']
    ...
    def online_train(self, cur_iter, batch_size, n_worker, seed):
        ...
        print("streamed class count")
        print(Counter(pd.DataFrame(self.streamed_list)['klass']))
        print(Counter(pd.DataFrame(self.streamed_list)['label']))
        ...
        # 지금까지 등장한 애들만 test data set에 나오도록 
        ...
        for test in self.test_list:
            if test['label'] in self.exposed_classes:
                real_test_list.append(test)
        print("test_len", len(self.test_list))    
        print("real_test_len", len(real_test_list))
        print(Counter(exp_test_df['klass']))
        print(Counter(exp_test_df['label']))
        self.test_list = real_test_list
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            # online training을 하면서 바로 memory update 
    ...
    def puridiver_sampling(self, cur_data_list):
        ...
        while len(cand_df) > self.memory_size:
            ...
            # middle value 구하기 위한 setting
            # sorted_score = sorted(score)
            # median_value = sorted_score[len(sorted_score)//2]
            # drop_idx = np.where(score==median_value)[0].item()
    ...
    def reservoir_sampling(self, samples):
        for sample in samples:
            if len(self.memory_list) < self.memory_size:
                ...
                # self.seen은 지금까지 본 sample의 수
                # def of reservoir sampling) 새로 들어온 sample이 memory에 들어갈 확률 : 1/N (seen sample 수)
                # but memory buffer size가 M이라면, M/N일 것이다.
